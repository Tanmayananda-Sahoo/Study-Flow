...
const SEED_CLASSES = [
  { id: 1,  day: 'Monday',    start: '09:00', end: '10:30', name: 'Advanced Math',  venue: 'A-204',   color: 'blue',   repeats: true  },
  { id: 2,  day: 'Monday',    start: '14:00', end: '16:00', name: 'Physics Lab',    venue: 'Lab D-1', color: 'orange', repeats: false },
  { id: 3,  day: 'Tuesday',   start: '10:00', end: '12:00', name: 'Organic Chem',   venue: 'B-108',   color: 'green',  repeats: true  },
  { id: 4,  day: 'Tuesday',   start: '13:00', end: '14:00', name: 'Statistics',     venue: 'A-101',   color: 'blue',   repeats: true  },
  { id: 5,  day: 'Wednesday', start: '08:00', end: '10:00', name: 'Calculus',       venue: 'C-203',   color: 'orange', repeats: true  },
  { id: 6,  day: 'Wednesday', start: '12:00', end: '14:00', name: 'English Lit',    venue: 'C-301',   color: 'green',  repeats: false },
  { id: 7,  day: 'Thursday',  start: '09:00', end: '12:00', name: 'Research Lab',   venue: 'Lab E-2', color: 'purple', repeats: true  },
  { id: 8,  day: 'Thursday',  start: '14:00', end: '16:00', name: 'Seminar',        venue: 'Hall B',  color: 'blue',   repeats: false },
  { id: 9,  day: 'Friday',    start: '10:00', end: '12:00', name: 'Data Science',   venue: 'Lab C-3', color: 'orange', repeats: true  },
  { id: 10, day: 'Friday',    start: '13:00', end: '14:00', name: 'Office Hours',   venue: 'Prof. Rm',color: 'green',  repeats: false },
]

const EMPTY_FORM = { title: '', day: '', start: '', end: '', venue: '', repeats: false }
...
export default function TimetablePage() {
  ...
  const toggle = ()      =>       setForm(prev => ({ ...prev, repeats: !prev.repeats }))
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4 pointer-events-none">
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-border-soft">
              <div>
                <h2 className="text-lg font-semibold text-ink tracking-tight">Add Class</h2>
                <p className="text-xs text-ink-muted mt-0.5">Schedule a new class in your timetable</p>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              ...
              {/* Title */}
              <div>
                <label className="block text-xs font-medium text-ink-muted mb-1.5 tracking-label">Class title</label>
                <input
                  type="text"
              ...
              {/* Day */}
              <div>
                <label className="block text-xs font-medium text-ink-muted mb-1.5 tracking-label">Day</label>
                <select
                  value={form.day}
                  onChange={set('day')}
              ...
              {/* Venue */}
              <div>
                <label className="block text-xs font-medium text-ink-muted mb-1.5 tracking-label">Venue / Room</label>
                <input
                  type="text"
              ...
              >
                ...
                <div
                  className={`
                    ...
                    ${form.repeats ? 'bg-accent border-accent' : 'bg-white border-border'}
                  `}
                >
                  {form.repeats && (
                ...
                <div>
                  <p className={`text-md font-medium ${form.repeats ? 'text-accent' : 'text-ink'}`}>
                    Repeat next week
                  </p>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-border-soft">
              ...
              <button onClick={handleAdd}  className="btn-primary flex-1">Add to Timetable</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <div className="flex items-start justify-between mb-0.5">
          <div>
            <h1 className="text-3xl font-semibold text-ink tracking-tighter">Timetable</h1>
          </div>
          <button
        ...
        <p className="text-md text-ink-muted mb-6">
          Spring Semester 2026 · {classes.length} classes · {repeatCount} repeating weekly
        </p>

        {/* Color legend */}
        <div className="flex gap-4 mb-5 flex-wrap">
          {classes.slice(0, 5).map(c => (
            <div key={c.id} className="flex items-center gap-1.5 text-xs text-ink-muted">
              <div className={`w-2.5 h-2.5 rounded-full ${
                c.color === 'blue'   ? 'bg-accent'      :
                c.color === 'orange' ? 'bg-orange-400'  :
                c.color === 'green'  ? 'bg-green-500'   :
                c.color === 'purple' ? 'bg-purple-400'  :
                                       'bg-rose-400'
              }`} />
              {c.name}
            </div>
          ...
          {classes.length > 5 && (
            <span className="text-xs text-ink-muted">+{classes.length - 5} more</span>
          )}
        </div>
